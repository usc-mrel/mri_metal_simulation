function [im, params, ks, noise_params] = simulateMAVRICSL(t1map, t2map, pdmap, dfmap, params)
% SIMULATEMAVRICSL Simulates MAVRIC-SL MRI sequence for metal implants
%
% Inputs:
...
for bin = 1:params.Nbins
    ...
    tic;
    fprintf('Simulating MAVRIC-SL bins: %d of %d\n', bin, params.Nbins);
    ...
    sig_prof = single(gaussianPulse(df_exc - cfreq, params));
    
    % Calculate excited signal
    sig_exc = sig_base .* sig_prof;
    
    % Phase change due to df, Gx and bin central freq
...
fprintf('Total MAVRIC-SL simulation time: %.2f seconds\n', total_time);
